...
@interface LYQRcode ()<AVCaptureMetadataOutputObjectsDelegate>

@property(nonatomic,assign,readonly)CGRect scanViewFrame;
@end
@implementation LYQRcode
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    if (metadataObjects.count>0) {
        ...
        if ([_delegate respondsToSelector:@selector(qrcode:ScanResult:)]) {
            [_delegate qrcode:self ScanResult:metadataObject.stringValue];
        }
    }
}
